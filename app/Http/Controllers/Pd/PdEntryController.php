...
class PdEntryController extends Controller
{
    public function show(Request $request, TournamentEvent $event): Response
    {
        ...
        $event->loadMissing(['sport:id,code,name', 'category:id,name,min_members,max_members']);

        $entries = EventEntry::query()
            ->with(['teams.members:id,event_entry_id,entry_team_id,pdam_id,name,identity_type,identity_number,identity_hash,documents,verification_status,verification_note,verified_at,updated_at', 'teams.members.pdam:id,name', 'members' => fn ($query) => $query->where('member_type', 'official')->select('id', 'event_entry_id', 'name', 'normalized_name', 'identity_type', 'identity_number', 'identity_hash', 'position', 'documents', 'updated_at')])
        ...
        $entries = $entries
            ->map(fn ($entry) => [
                ...
                'teams' => $entry->teams->map(fn ($team) => ['id' => $team->id, 'team_no' => $team->team_no, 'label' => $team->label, 'members' => $team->members->map(fn ($member) => $this->memberPayload($member)), 'status' => $team->effectiveStatus(), 'override' => $team->verification_status_override, 'verification_note' => $team->verification_note, 'editable' => $team->verification_status_override === 'revision_required']),
        ...
        return Inertia::render('Pd/EventEntries', [
            'event' => [
                ...
                'format_label' => \App\Models\Sport::FORMAT_LABELS[$rules['format'] ?? $event->format] ?? Str::headline($rules['format'] ?? $event->format),
                'sport' => $event->sport?->name,
            ],
            'category' => $event->category ? [
                'name' => $rules['category_name'] ?? $event->category->name,
                'min_members' => $rules['min_members_per_team'] ?? $rules['min_members'] ?? $event->category->min_members,
                'max_members' => $rules['max_members_per_team'] ?? (array_key_exists('max_members', $rules) ? $rules['max_members'] : $event->category->max_members),
