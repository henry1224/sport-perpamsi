# Regulation Reference v1

## Prinsip

- Regulasi final tetap diputuskan melalui technical meeting panitia.
- Dokumen ini memberi referensi resmi/otoritatif untuk baseline sistem.
- Sistem menyimpan hasil dan bracket sesuai regulasi yang dipilih, bukan menghitung semua detail aturan federasi secara otomatis.
- Admin menetapkan regulasi final pada paket kompetisi sebelum publikasi registrasi.
- Portal PD tidak memilih regulasi; PD hanya memilih kompetisi terpublikasi dan mengikuti snapshot regulasinya.
- Perubahan master regulasi setelah publikasi tidak mengubah kompetisi berjalan.

## Referensi per Cabor

| Cabor | Referensi | Yang Dipakai Sistem v1 |
|---|---|---|
| Voli | FIVB Official Volleyball Rules | Set, poin per set, match point, set ratio, point ratio |
| Catur | FIDE Laws of Chess dan Tie-break Regulations | Match point/game point, direct encounter, Buchholz/Sonneborn-Berger bila dipakai |
| Bulu tangkis | BWF Statutes/Laws of Badminton | Match/game/set, poin per game, head-to-head, selisih game/poin |
| Tenis meja | ITTF Handbook/Laws of Table Tennis | Match/game, poin per game, selisih game/poin |
| Tenis lapangan | ITF Rules of Tennis | Set, game, tie-break, walkover/retirement |
| Golf | Rules of Golf R&A/USGA | Stroke/score ranking, nilai terendah terbaik |
| Padel | FIP Rules of Padel | Set, game, tie-break |
| Mini Football | World Minifootball Federation rules | Gol, poin grup, selisih gol, head-to-head |
| Vokal | Regulasi panitia/technical meeting | Nilai juri, kategori penilaian, ranking nilai tertinggi |

## Link Referensi

- FIVB Official Volleyball Rules: https://www.fivb.com/volleyball/the-game/official-volleyball-rules/
- FIDE Laws of Chess: https://handbook.fide.com/chapter/E012023
- FIDE Tie-break Regulations: https://handbook.fide.com/chapter/TieBreakRegulations032026
- BWF Statutes/Laws: https://corporate.bwfbadminton.com/statutes/
- ITTF Handbook: https://www.ittf.com/handbook/
- ITF Rules of Tennis: https://www.itftennis.com/en/about-us/governance/rules-and-regulations/
- Rules of Golf R&A: https://www.randa.org/rog/the-rules-of-golf
- USGA Rules of Golf: https://www.usga.org/rules/rules-and-clarifications/rules-of-golf.html
- FIP Rules of Padel: https://www.padelfip.com/rules/
- World Minifootball Federation Rules: https://www.minifootball.com/pages/rules

## Gap Regulasi yang Harus Dikunci Panitia

- Apakah tiap cabor memakai grup, knockout, round robin, Swiss, atau double elimination.
- Jumlah peserta per tim dan cadangan.
- Jumlah set/game/babak resmi per cabor.
- Aturan walkover, diskualifikasi, cedera, dan keterlambatan.
- Tie-break final jika semua indikator sama.
- Apakah golf, padel, dan vokal dihitung ke medali/ranking umum atau hanya eksibisi.
- Batas peserta Golf per PD karena slide 21 hanya menyatakan biaya per orang tanpa kuota maksimum.
- Konfirmasi teks Ganda Campuran Tenis Meja pada slide 11 yang tertulis `1 Orang`; baseline sistem memakai satu pasangan atau dua pemain.
- Konfirmasi kuota Tenis Lapangan karena setiap kategori tertulis tiga pasangan, sedangkan ringkasan PD juga menyebut maksimal tiga pasangan.
