...
|---|---|---|---|---|
| `users.role` | `super_admin`, `admin_event`, `pd_admin`, `scorekeeper`, `sport_coordinator` | Super Admin, Admin, Pengurus Daerah, Scorekeeper, Koordinator Cabor | Konvensi kode; belum ada DB CHECK | Role target lain (`verifikator_peserta`, `content_officer`, `auditor`) belum tersedia. Jangan tampilkan sebagai role aktif. |
| `users.account_status` | `pending`, `revision_required`, `verified`, `rejected`, `suspended` | Menunggu Verifikasi, Perlu Perbaikan, Aktif, Ditolak, Nonaktif | Konvensi kode; default DB `verified`; belum ada DB CHECK | Akun internal Admin/Panitia dibuat langsung `verified` dan dapat dinonaktifkan sebagai `suspended`; akun PD mengikuti workflow registrasi daerah. |
...
| `tournament_events.status` | `registration_draft`, `registration_open`, `registration_closed`, `participants_locked`, `bracket_locked`; kode/filter juga mengenal `ongoing`, `completed` | Draft, Pendaftaran Dibuka, Pendaftaran Ditutup, Peserta Dikunci, Bracket Dikunci, Sedang Berlangsung, Selesai | Konvensi kode; default DB `registration_draft`; belum ada DB CHECK | `participants_locked` dipakai format non-bracket; `bracket_locked` dipakai format bracket. Transition ke `ongoing` dan `completed` belum tersedia pada controller. `archived` masih target dokumen. |
...
| `participants_locked` | Peserta Dikunci | Informasi/biru | Peserta format non-bracket dikunci; registrasi tidak dapat berubah biasa dan seed tidak dibuat. |
