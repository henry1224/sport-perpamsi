...
import { formatDateTime, formatPeriodDate } from '../../lib/date';
...
const generateMatches = (event) => {
  if (!confirm(`Buat susunan pertandingan ${event.name}?`)) return;
  busy.value = event.code;
  router.post(`/admin/events/${event.code}/generate-matches`, {}, { preserveScroll: true, onFinish: () => { busy.value = null; } });
};
</script>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr v-for="event in group.events" v-show="isExpanded(group.name)" :key="event.code" class="event-row">
            ...
            <td><div class="registration-cell"><span :class="['status-badge', statusTone(event.status)]">{{ statusLabel(event.status) }}</span><small v-if="event.open_at">{{ formatPeriodDate(event.open_at) }}<template v-if="event.close_at"> — {{ formatPeriodDate(event.close_at) }}</template></small><small v-else>Belum dijadwalkan</small></div></td>
            ...
            <td><details class="action-menu" :open="openActionMenu === event.code" @click.stop><summary aria-label="Buka menu aksi" @click.prevent="toggleActionMenu(event.code)">•••</summary><div><button v-if="!event.published && event.entries_count === 0" type="button" @click="closeActionMenu(); openEvent(event)">Edit Data</button><button v-if="!event.published && event.entries_count === 0" class="primary-action" type="button" @click="closeActionMenu(); openPublish(event)">Buka Pendaftaran</button><button v-if="event.published && event.status === 'registration_open'" class="danger-action" type="button" :disabled="busy === event.code" @click="closeActionMenu(); close(event)">Tutup Pendaftaran</button><button v-if="event.status === 'registration_closed'" class="primary-action" type="button" :disabled="busy === event.code || !event.verification_complete" @click="closeActionMenu(); lockBracket(event)">Kunci Bracket</button><button v-if="event.status === 'bracket_locked' && event.matches_count === 0 && [2, 4].includes(event.verified_teams_count)" class="primary-action" type="button" :disabled="busy === event.code" @click="closeActionMenu(); generateMatches(event)">Buat Pertandingan</button><button v-if="event.published && event.entries_count === 0" type="button" :disabled="busy === event.code" @click="closeActionMenu(); unpublish(event)">Kembalikan ke Draft</button><button v-if="!event.published && event.entries_count === 0" class="danger-action" type="button" @click="closeActionMenu(); deleteEvent(event)">Hapus Data</button><small v-if="event.status === 'registration_closed'">{{ event.verification_complete ? 'Seluruh team dan pemain siap dikunci.' : `${event.players_count - event.verified_players_count} pemain dan ${event.teams_count - event.verified_teams_count} team belum selesai.` }}</small><small v-else-if="event.status === 'bracket_locked'">{{ event.matches_count ? `${event.matches_count} pertandingan siap dijadwalkan.` : 'Bracket terkunci. Buat pertandingan untuk melanjutkan ke Agenda & Jadwal.' }}</small><small v-else-if="event.status === 'ongoing'">Kelola pertandingan melalui menu Skor.</small><small v-else-if="event.status === 'completed'">Kompetisi selesai dan siap dilaporkan.</small></div></details></td>
          </tr>
          </template>
