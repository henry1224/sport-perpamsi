  ...
  <PortalLayout portal="admin">
    ...
    <div class="admin-columns">
      ...
      <section class="admin-panel access-panel">
        <header><div><span>Kontrol Akses</span><h2>Panitia</h2></div><Link href="/admin/assignments">Kelola</Link></header>
        ...
        <b>Assignment cabor dan venue sudah dapat dikelola.</b>
      </section>
    </div>
  </PortalLayout>
</template>
...
<style scoped>
.admin-hero { display: flex; align-items: end; justify-content: space-between; gap: 24px; padding: 30px; color: #fff; background: linear-gradient(125deg,#0b2448 0%,#123f78 68%,#17629a 100%); border: 1px solid #1f5790; border-radius: 16px; box-shadow: 0 18px 45px rgba(15,49,91,.18); }
.admin-hero span, .admin-panel header span { color: #55c7ed; font-size: 10px; font-weight: 800; letter-spacing: .14em; text-transform: uppercase; }
.admin-hero h1 { margin: 7px 0; font-size: clamp(28px, 4vw, 44px); line-height: 1.08; }
.admin-hero p { max-width: 650px; margin: 0; color: rgba(255,255,255,.72); line-height: 1.6; }
.admin-hero a { flex: none; padding: 11px 16px; color: #1946a3; background: #fff; border-radius: 8px; font-size: 12px; font-weight: 800; text-decoration: none; box-shadow: 0 6px 16px rgba(4,24,54,.18); }
.metric-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; margin: 28px 0; }
.metric-grid article { position: relative; display: grid; min-height: 145px; padding: 20px; overflow: hidden; background: #fff; border: 1px solid #d9e3e9; border-radius: 14px; box-shadow: 0 8px 24px rgba(25,53,76,.06); }
.metric-grid small { position: absolute; right: 12px; top: 4px; color: #edf2f4; font-size: 58px; font-weight: 1000; }
.metric-grid span { position: relative; color: #63727e; font-size: 10px; font-weight: 800; letter-spacing: .1em; text-transform: uppercase; }
...
.admin-panel { padding: 22px; background: #fff; border: 1px solid #d9e3e9; border-radius: 14px; box-shadow: 0 8px 24px rgba(25,53,76,.06); }
...
.admin-panel header a { color: #1946a3; font-size: 12px; font-weight: 800; text-decoration: none; }
